...
bool Load(const char* fileName, std::vector<Polygon>& Facets){
...
int main(){
	...
	stbi_write_png("result1.png", 256, 256, 4, image, 256 * 4);
	...
	for (auto s : Facets){
		Spheres.push_back(Sphere(s.GetCenter(), 0.006));
	}


	std::cout << "Create OBVH Start" << std::endl;
	OBVH<Sphere> sGraph;
	sGraph.CreateGraph(Spheres);
	std::cout << "Create OBVH End" << std::endl;
	...
	for (int i = 0; i < 256 * 256; i++){
		...
		Ray ray(Vec3(rx, ry, -10), Vec3(0, 0, 1));
		std::vector<OBVH<Sphere>::IscData> result;

		int ss = sGraph.GetElementFromRay(ray, result);

		Vec3 Normal(0, 0, 0);
		if (ss > 0){
			Vec3 Center = result[0]._pElement->m_Pos;
			Vec3 HitPos = ray.m_Org + ray.m_Dir * result[0]._d;
			Normal = (HitPos - Center).normalized();
		}
	...
	stbi_write_png("result2.png", 256, 256, 4, image, 256 * 4);
